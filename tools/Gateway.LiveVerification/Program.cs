...
try
{
    ...
    string controlScope;
    if (options.AuthenticationMode == VerificationAuthenticationMode.InteractiveBrowserUser)
    {
    ...
    ControlTokenValidator.Validate(
        ...
        options.TenantId,
        options.AuthenticationMode == VerificationAuthenticationMode.InteractiveBrowserUser,
    ...
    if (options.OperationMode == VerificationOperationMode.RevokeOnly)
    {
    ...
    else
    {
                   ...
                   content: null))
        {
            ...
            VerificationEvidenceValidator.ValidateIssuedCredential(
                issued,
        ...
        var interactionId = $"prompt-shield-verification-{suffix}";
        ...
        var allowed = VerificationEvidenceValidator.ValidateAllowedEvaluation(await EvaluateAsync(
            dataClient,
            options,
            interactionId,
            occurredAtUtc,
        ...
        new
        {
            ...
            {
                ["verification"] = "prompt-shield-verification",
                ["transport"] = "gateway"
            }
        },
        ...
        new
        {
            ...
            response = new
            {
                ...
                content = "The operator verification received the safe message."
            },
            model = (object?)null,
        ...
        if (options.ExpectPromptShieldEnabled)
        {
            ...
            var blocked = VerificationEvidenceValidator.ValidateBlockedEvaluation(await EvaluateAsync(
                dataClient,
                options,
        ...
        Console.WriteLine("[PASS] Live Gateway verification completed.");
        exitCode = 0;
    }
}
catch (Exception)
{
    Console.Error.WriteLine("[FAILED] Live verification operation failed; provider details were suppressed.");
}
finally
{
    ...
    if (credentialId != Guid.Empty)
    {
        try
        {
            ...
            VerificationEvidenceValidator.ValidateRevokedCredential(
                revoked,
                options.AgentRegistrationId,
                credentialId);
...
static async Task SendAcceptedAsync(HttpClient client, string path, object body, string label)
{
    ...
    var correlationId = VerificationEvidenceValidator.RequireHeaderCorrelation(ReadCorrelation(response));
...
internal sealed record Options(
    ...
    Guid TenantId,
    VerificationAuthenticationMode AuthenticationMode,
    Guid? AuthenticationClientId,
    VerificationOperationMode OperationMode,
    ...
    bool ExpectPurviewEnabled)
{
    public static Options Parse(string[] args)
    {
        ...
        {
            "InteractiveBrowserUser" => VerificationAuthenticationMode.InteractiveBrowserUser,
            "ManagedIdentityApplication" => VerificationAuthenticationMode.ManagedIdentityApplication,
            _ => throw new ArgumentException(
                "--authentication-mode must be InteractiveBrowserUser or ManagedIdentityApplication.")
        };
        Guid? authenticationClientId = null;
        if (authenticationMode == VerificationAuthenticationMode.InteractiveBrowserUser)
        {
        ...
        {
            "Full" => VerificationOperationMode.Full,
            "RevokeOnly" => VerificationOperationMode.RevokeOnly,
            _ => throw new ArgumentException("--operation-mode must be Full or RevokeOnly.")
        };
        Guid? recoveryCredentialId = null;
        if (operationMode == VerificationOperationMode.RevokeOnly)
        {
...
internal enum VerificationAuthenticationMode
{
    InteractiveBrowserUser,
    ManagedIdentityApplication
}

internal enum VerificationOperationMode
{
    Full,
    RevokeOnly
}
