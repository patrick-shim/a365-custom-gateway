...
<#
.SYNOPSIS
    Verifies one Active registration as the assigned Gateway Administrator.

.DESCRIPTION
    This optional post-deployment check is not part of bootstrap completion and is
    not a release gate. It creates one registration-bound, credential-free public
    client and one principal-specific access_as_user grant, invokes
    Gateway.LiveVerification through
...
$helperManifest = @($helperRelativePaths | Sort-Object | ForEach-Object {
    ...
    if (-not $file.PSIsContainer -and
        ...
        throw 'The first-registration verification helper bundle contains a missing, redirected, or out-of-root source file.'
    }
}) -join "`n"
...
Import-Module (Join-Path $repositoryRoot 'operations/FirstRegistrationVerificationState.psm1') -Force

$verificationStateFileName = "$ExpectedSubscriptionId-$AgentRegistrationId.json"
$verificationStatePath = Join-Path $repositoryRoot ".bootstrap/verification/$verificationStateFileName"
...
function Get-VerificationGrants {
...
function Invoke-ExactVerificationGraphGetOrNull {
...
function Get-ExactVerificationApplicationById {
...
function Wait-ExactVerificationObjectAbsent {
    param(
...
function Wait-ExactApplicationByName {
    ...
    for ($attempt = 1; $attempt -le 18; $attempt++) {
        ...
        if ($applications.Count -gt 1) {
            throw 'The execution-bound verification application name resolved ambiguously.'
        }
...
function Wait-ExactGrantByClientId {
    ...
    for ($attempt = 1; $attempt -le 18; $attempt++) {
        $grants = @(Get-VerificationGrants -ClientServicePrincipalId $ClientServicePrincipalId)
        if ($grants.Count -gt 1) {
            throw 'The execution-bound verification delegated grant resolved ambiguously.'
        }
...
function Assert-ExactTemporaryApplication {
    ...
    Assert-ExactApplicationAuthenticationSurface `
        -Application $Application `
        -ApplicationLabel 'Temporary verification application' | Out-Null
    ...
    if ([string]$Application.displayName -cne $ExpectedDisplayName -or
        ...
        throw 'The temporary verification application does not match the exact credential-free public-client boundary.'
    }
}
...
function Assert-ExactTemporaryApplicationOwner {
    ...
    if ($ownerIds.Count -ne 1 -or
        ...
        throw 'The temporary verification application must have exactly the pinned administrator as owner.'
    }
}
...
function Assert-ExactTemporaryServicePrincipal {
    ...
    Assert-ExactBootstrapServicePrincipalBoundary `
        ...
        -ServicePrincipalLabel 'Temporary verification service principal' `
...
function Assert-ExactTemporaryGrant {
    ...
    if ([string]$Grant.clientId -cne $ExpectedClientServicePrincipalId -or
        ...
        throw 'The temporary verification delegated grant does not match the exact user and access_as_user boundary.'
    }
}
...
function Remove-ExactTemporaryGrant {
    ...
    $exactGrant = Get-ExactVerificationGrantById -GrantId $temporaryGrantId
    $grants = @(Get-VerificationGrants -ClientServicePrincipalId $temporaryServicePrincipalId)
    if ($null -eq $exactGrant) {
        ...
        Wait-ExactVerificationObjectAbsent `
            -ReadExact { Get-ExactVerificationGrantById -GrantId $temporaryGrantId } `
    ...
    if (@(Get-VerificationGrants -ClientServicePrincipalId $temporaryServicePrincipalId).Count -ne 0) {
...
function Remove-ExactTemporaryServicePrincipal {
    ...
    $exactPrincipal = Get-ExactVerificationServicePrincipalById -ServicePrincipalId $temporaryServicePrincipalId
    ...
    if ($null -eq $exactPrincipal) {
        ...
        Wait-ExactVerificationObjectAbsent `
            -ReadExact { Get-ExactVerificationServicePrincipalById -ServicePrincipalId $temporaryServicePrincipalId } `
...
function Remove-ExactTemporaryApplication {
    ...
    $exactApplication = Get-ExactVerificationApplicationById -ApplicationObjectId $temporaryApplicationId
    ...
    if ($null -eq $exactApplication) {
        ...
        Wait-ExactVerificationObjectAbsent `
            -ReadExact { Get-ExactVerificationApplicationById -ApplicationObjectId $temporaryApplicationId } `
...
function Adopt-ExactTemporaryAuthority {
    ...
    if ($null -ne $Application) {
        ...
        if ((-not [string]::IsNullOrEmpty($recordedApplicationId) -and
            ...
            throw 'The temporary application does not match its durable verification-state binding.'
        }
    ...
    elseif ($RequireComplete) {
        throw 'The complete temporary verification application was not observable.'
    }
    else {
    ...
    if ($null -ne $principal) {
        ...
        if (-not [string]::IsNullOrEmpty($recordedPrincipalId) -and
            ...
            throw 'The temporary service principal does not match its durable verification-state binding.'
        }
        $script:temporaryServicePrincipalId = $observedPrincipalId
    }
    elseif ($RequireComplete) {
        throw 'The complete temporary verification service principal was not observable.'
    }
    else {
    ...
    $grants = @(Get-VerificationGrants -ClientServicePrincipalId $script:temporaryServicePrincipalId)
    if ($grants.Count -gt 1) {
        throw 'The temporary verification delegated grant resolved ambiguously.'
    }
    if ($grants.Count -eq 1) {
        ...
        if (-not [string]::IsNullOrEmpty($recordedGrantId) -and
            ...
            throw 'The temporary delegated grant does not match its durable verification-state binding.'
        }
...
$identityStem = "A365 Gateway First Registration Verification - $ProjectName-$Environment-$registrationToken"
$preChildDisplayName = "$identityStem - PreChild"
$childArmedDisplayName = "$identityStem - ChildArmed"
$executionTag = "a365gw:first-registration-verification:$registrationToken"
$verificationOutput = Join-Path $repositoryRoot 'tools/Gateway.LiveVerification/bin/Release/net10.0'
$verificationAssembly = Join-Path $verificationOutput 'Gateway.LiveVerification.dll'
if (-not (Test-Path -LiteralPath $verificationAssembly -PathType Leaf)) {
    throw 'The reviewed Release Gateway.LiveVerification assembly is absent. Build it before any live verification action.'
}
$wrapperSha256 = "sha256:$((Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant())"
$verificationRuntimeFiles = @(Get-ChildItem -LiteralPath $verificationOutput -Recurse -File | Where-Object {
    $_.Extension -ceq '.dll' -or
    $_.Name -ceq 'Gateway.LiveVerification.deps.json' -or
    $_.Name -ceq 'Gateway.LiveVerification.runtimeconfig.json'
} | ForEach-Object {
    $relativePath = [IO.Path]::GetRelativePath($verificationOutput, $_.FullName).Replace('\', '/')
    [pscustomobject]@{ File = $_; RelativePath = $relativePath }
} | Sort-Object RelativePath)
foreach ($requiredRuntimeFile in @(
    'Gateway.LiveVerification.dll',
    'Gateway.LiveVerification.deps.json',
    'Gateway.LiveVerification.runtimeconfig.json'
)) {
    if (@($verificationRuntimeFiles | Where-Object RelativePath -CEQ $requiredRuntimeFile).Count -ne 1) {
        throw 'The reviewed Release Gateway.LiveVerification runtime bundle is incomplete or ambiguous.'
    }
}
$verificationBundleManifest = @($verificationRuntimeFiles | ForEach-Object {
    if ($_.RelativePath.Length -gt 512 -or
        ...
        throw 'The reviewed Release Gateway.LiveVerification runtime bundle contains an unsafe file entry.'
    }
    "$($_.RelativePath)=sha256:$((Get-FileHash -LiteralPath $_.File.FullName -Algorithm SHA256).Hash.ToLowerInvariant())"
}) -join "`n"
$verificationBundleSha256 = Get-BootstrapSha256 -Text $verificationBundleManifest
$verificationBindings = [ordered]@{
    ...
    verificationBundleSha256 = $verificationBundleSha256
    preChildDisplayName = $preChildDisplayName
    childArmedDisplayName = $childArmedDisplayName
    executionTag = $executionTag
}
Assert-FirstRegistrationVerificationBindings -Bindings $verificationBindings

try {
    $verificationLock = Enter-BootstrapLock -StatePath $verificationStatePath
    $verificationState = Read-FirstRegistrationVerificationState -Path $verificationStatePath -Bindings $verificationBindings
    if ($null -eq $verificationState) {
        if ($recoveryMode) {
            throw 'RevokeOnly requires the exact durable verification state that observed this credential ID.'
        }
        $verificationState = New-FirstRegistrationVerificationState -Bindings $verificationBindings
        Save-FirstRegistrationVerificationState -State $verificationState -Path $verificationStatePath -Bindings $verificationBindings
    }

    if (Test-FirstRegistrationVerificationStateRequiresPreservation -State $verificationState -Bindings $verificationBindings) {
        $preserveTemporaryIdentity = $true
    }
    if (@('ArmStarted', 'ChildArmed', 'ChildLaunchStarted', 'CredentialObserved') -ccontains
        [string]$verificationState.status) {
        $recoveryBoundaryArmed = $true
    }

    if ([string]$verificationState.status -ceq 'Completed') {
        if ($recoveryMode) {
            throw 'The exact first-registration verification is already Completed; a second recovery revocation is not authorized.'
        }
        $completedTombstone = $true
    }
    elseif ($recoveryMode) {
        if ([string]$verificationState.status -cne 'CredentialObserved' -or
            [string]$verificationState.recoveryCredentialId -cne $RecoveryCredentialId) {
            throw 'RevokeOnly requires the exact credential ID durably observed for this registration-bound verification.'
        }
    }
    elseif (@('ChildLaunchStarted', 'CredentialObserved') -ccontains [string]$verificationState.status) {
        throw 'The verification crossed the durable child-launch boundary. Full mode is forbidden; use the exact recorded recovery path or manual credential inspection.'
    }
    ...
    if ([string]$azureIdentity.subscriptionId -cne $ExpectedSubscriptionId -or
        ...
        throw 'The Azure CLI could not be pinned to the exact verification subscription and tenant.'
    }
    ...
    if ([string]$containerApp.properties.configuration.ingress.fqdn -cne $parsedApiBaseUrl.Host -or
        ...
        throw 'The verification API URL does not match the exact externally reachable Gateway API Container App.'
    }
    ...
    if ($preChildApplications.Count -gt 1 -or
        ...
        throw 'The registration-bound verification application stage resolved ambiguously.'
    }

    if ($completedTombstone) {
        if ($preChildApplications.Count -ne 0) {
            throw 'Completed verification state conflicts with a PreChild temporary application; refusing cleanup.'
        }
        $preserveTemporaryIdentity = $true
        $completedApplication = if ($childArmedApplications.Count -eq 1) {
            ...
            Get-ExactVerificationApplicationById `
                -ApplicationObjectId ([string]$verificationState.temporaryApplicationObjectId)
        }
        ...
        Write-Host '[PASS] The registration-bound verification is already Completed; only exact leftover Entra cleanup will run.'
    }
    elseif ($recoveryMode) {
        if ($preChildApplications.Count -ne 0 -or $childArmedApplications.Count -ne 1) {
            throw 'RevokeOnly recovery requires exactly one ChildArmed registration-bound verification application.'
        }
        Adopt-ExactTemporaryAuthority `
        ...
        if ([string]$verificationState.status -ceq 'Prepared') {
            if ($preChildApplications.Count -ne 0 -or $childArmedApplications.Count -ne 0) {
                throw 'Prepared verification state conflicts with an existing registration-bound temporary application.'
            }
            $verificationState = Set-FirstRegistrationVerificationStateStatus `
                -State $verificationState `
        ...
        if ([string]$verificationState.status -ceq 'ApplicationCreateStarted') {
            $preserveTemporaryIdentity = $true
        ...
        if ([string]$verificationState.status -ceq 'ApplicationObserved') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            ...
            $temporaryApplicationId = [string]$temporaryApplication.id
            $temporaryApplicationClientId = [string]$temporaryApplication.appId
            if ($temporaryApplicationId -cne [string]$verificationState.temporaryApplicationObjectId -or
                $temporaryApplicationClientId -cne [string]$verificationState.temporaryApplicationClientId -or
        ...
        if ([string]$verificationState.status -ceq 'OwnerAddStarted') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                [string]$temporaryApplication.id -cne [string]$verificationState.temporaryApplicationObjectId -or
                [string]$temporaryApplication.appId -cne [string]$verificationState.temporaryApplicationClientId -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'OwnerObserved') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'ServicePrincipalCreateStarted') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'ServicePrincipalObserved') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'GrantCreateStarted') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'AuthorityReady') {
            $temporaryApplication = Wait-ExactApplicationByName -DisplayName $preChildDisplayName
            if (-not $temporaryApplication -or
                @(Get-ApplicationsByExactDisplayName -DisplayName $childArmedDisplayName).Count -ne 0) {
        ...
        if ([string]$verificationState.status -ceq 'ArmStarted') {
            $recoveryBoundaryArmed = $true
            $preserveTemporaryIdentity = $true
        ...
        if ([string]$verificationState.status -ceq 'ChildArmed') {
            $recoveryBoundaryArmed = $true
            $preserveTemporaryIdentity = $true
        ...
        if ([string]$verificationState.status -cne 'ChildArmed') {
            throw 'The full verification did not reach the exact durable ChildArmed stage.'
        }
    }
    ...
    if (-not $completedTombstone) {
        ...
        if ($recoveryMode) {
            $verificationArguments += @('--recovery-credential-id', $RecoveryCredentialId)
        }

        & dotnet @verificationArguments 2>&1 | ForEach-Object {
            $childLine = [string]$_
            if ($childLine -cmatch '^\[PASS\] Temporary registration-bound credential issued: key ID ([0-9a-f-]{36})\.$') {
                if ($script:recoveryMode -or [string]$script:verificationState.status -cne 'ChildLaunchStarted') {
                    throw 'The verification child emitted a credential outside the exact Full child-launch boundary.'
                }
                ...
                $script:verificationState = Set-FirstRegistrationVerificationStateStatus `
                    -State $script:verificationState `
                    -Bindings $script:verificationBindings `
                    -Status 'CredentialObserved' `
                    -RecoveryCredentialId $candidateCredentialId
                Save-FirstRegistrationVerificationState `
                    -State $script:verificationState `
                    -Path $script:verificationStatePath `
                    -Bindings $script:verificationBindings
        ...
        if ($childExitCode -ne 0) {
            if ([string]$verificationState.status -ceq 'CredentialObserved') {
                throw "The first-registration verification failed after key ID $($verificationState.recoveryCredentialId) was durably observed. The temporary auth identity was preserved; rerun with that exact -RecoveryCredentialId value."
            }
            throw 'The first-registration verification failed after ChildLaunchStarted without a durably observed key ID. The temporary auth identity was preserved; do not issue another key. Inspect the registration credential lifecycle and perform exact manual recovery.'
        }

        if ([string]$verificationState.status -cne 'CredentialObserved') {
        ...
        if ($recoveryMode) {
            ...
            Write-Host '[PASS] Bounded interactive-user verification completed; temporary Entra cleanup is starting.'
        }
    }
}
catch {
    ...
    try {
        ...
        elseif ($recoveryBoundaryArmed) {
            Write-Warning '[RECOVERY REQUIRED] The registration-bound verification authority was preserved because exact Gateway-key revocation was not proven.'
        }
        else {
            Write-Warning '[RESUME REQUIRED] Exact PreChild verification authority was preserved at a durable GET-only stage; its external mutation will not be replayed.'
        }
        Clear-BootstrapAzureSubscriptionContext
    }
    finally {
        if ($null -ne $verificationLock) {
...
Write-Host '[PASS] Temporary verification grant, service principal, and active application are absent.'
