...
public class EnableAgentHandlerTests
{
    ...
    [Fact]
    public async Task Handle_Should_ReturnAgentStateChangeResponse_When_AgentIsDisabled()
    {
        var agent = CreateAgent(AgentStatus.Disabled);
        var command = new EnableAgentCommand(agent.Id, "caller-oid-001");
        ...
        result.Status.Should().Be(AgentStatus.Active.ToString());
    }

    [Fact]
    public async Task Handle_Should_ChangeStatusToActive_When_AgentIsDisabled()
    {
        var agent = CreateAgent(AgentStatus.Disabled);
        var command = new EnableAgentCommand(agent.Id, "caller-oid-001");
    ...
    [Fact]
    public async Task Handle_Should_CreateAuditEvent_When_AgentEnabled()
    {
        var agent = CreateAgent(AgentStatus.Disabled);
        var command = new EnableAgentCommand(agent.Id, "caller-oid-001");
    ...
    [Fact]
    public async Task Handle_Should_CallSaveChanges_When_AgentEnabled()
    {
        var agent = CreateAgent(AgentStatus.Disabled);
        var command = new EnableAgentCommand(agent.Id, "caller-oid-001");
    ...
    {
        var command = new EnableAgentCommand(Guid.NewGuid(), "caller-oid-001");
    ...
    public async Task Handle_Should_ThrowInvalidStateTransitionException_When_AgentIsNotDisabled(AgentStatus status)
    {
        ...
        exception.Which.AttemptedAction.Should().Be("Enable");
    }
}
