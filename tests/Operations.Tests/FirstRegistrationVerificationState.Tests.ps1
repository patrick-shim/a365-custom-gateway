Describe 'First-registration verification durable state' {
    BeforeAll {
        $script:RepositoryRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
        $script:StateModulePath = Join-Path $script:RepositoryRoot 'operations/FirstRegistrationVerificationState.psm1'
        Import-Module $script:StateModulePath -Force

        function New-TestVerificationBindings {
            return [ordered]@{
                subscriptionId = '11111111-1111-1111-1111-111111111111'
                tenantId = '22222222-2222-2222-2222-222222222222'
                projectName = 'gatewaydemo'
                environment = 'dev'
                resourceGroup = 'rg-a365-gateway-verification'
                ...
                verificationBundleSha256 = "sha256:$('b' * 64)"
                preChildDisplayName = 'A365 Gateway First Registration Verification - gatewaydemo-dev-44444444444444444444444444444444 - PreChild'
                childArmedDisplayName = 'A365 Gateway First Registration Verification - gatewaydemo-dev-44444444444444444444444444444444 - ChildArmed'
                executionTag = 'a365gw:first-registration-verification:44444444444444444444444444444444'
            }
        }

        function Move-TestVerificationToChildLaunch {
            param(
                [Parameter(Mandatory)][System.Collections.IDictionary]$State,
                [Parameter(Mandatory)][System.Collections.IDictionary]$Bindings
            )

            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'ApplicationCreateStarted'
            $State = Set-FirstRegistrationVerificationStateStatus `
                -State $State `
                -Bindings $Bindings `
            ...
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'OwnerAddStarted'
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'OwnerObserved'
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'ServicePrincipalCreateStarted'
            $State = Set-FirstRegistrationVerificationStateStatus `
                -State $State `
                -Bindings $Bindings `
                -Status 'ServicePrincipalObserved' `
                -TemporaryServicePrincipalId '88888888-8888-8888-8888-888888888888'
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'GrantCreateStarted'
            $State = Set-FirstRegistrationVerificationStateStatus `
                -State $State `
                -Bindings $Bindings `
                -Status 'AuthorityReady' `
                -TemporaryGrantId 'safe_grant_id'
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'ArmStarted'
            $State = Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'ChildArmed'
            return Set-FirstRegistrationVerificationStateStatus -State $State -Bindings $Bindings -Status 'ChildLaunchStarted'
        }
    }

    BeforeEach {
        $script:Bindings = New-TestVerificationBindings
        $script:State = New-FirstRegistrationVerificationState -Bindings $script:Bindings
    }
    ...
    It 'creates a fixed safe-identifier Prepared state that does not require preservation' {
        ...
        $script:State.schemaVersion | Should -Be 3
        ...
        $script:State.verificationBundleSha256 | Should -BeExactly $script:Bindings.verificationBundleSha256
    ...
    It 'requires canonical lowercase protection expectations in the durable binding' {
        $uppercase = New-TestVerificationBindings
        $uppercase.promptShieldExpected = 'False'
        { New-FirstRegistrationVerificationState -Bindings $uppercase } |
            Should -Throw '*bounded non-secret contract*'

        $missing = New-TestVerificationBindings
        $missing.Remove('purviewExpected')
        { New-FirstRegistrationVerificationState -Bindings $missing } |
            Should -Throw '*unexpected property set*'
    }

    It 'round-trips atomically without temporary files or credential material' {
        $path = Join-Path $TestDrive 'nested/verification.json'
        Save-FirstRegistrationVerificationState -State $script:State -Path $path -Bindings $script:Bindings
        $loaded = Read-FirstRegistrationVerificationState -Path $path -Bindings $script:Bindings
    ...
    It 'normalizes PowerShell 7.0 through 7.4 automatic JSON dates before validation' {
        ...
        InModuleScope FirstRegistrationVerificationState -Parameters @{ Parsed = $legacyParsed } {
            param($Parsed)
            Convert-VerificationParsedJsonDatesToStrings -Value $Parsed
        }
        $legacyParsed.createdAtUtc | Should -BeOfType [string]
        $legacyParsed.createdAtUtc | Should -BeExactly $script:State.createdAtUtc
        { Assert-FirstRegistrationVerificationState -State $legacyParsed -Bindings $script:Bindings } |
            Should -Not -Throw
    }

    It 'supports only the ordered per-mutation Started and Observed path' {
        $script:State = Move-TestVerificationToChildLaunch -State $script:State -Bindings $script:Bindings
        $script:State.status | Should -BeExactly 'ChildLaunchStarted'
        Test-FirstRegistrationVerificationStateRequiresPreservation `
            -State $script:State `
            -Bindings $script:Bindings | Should -BeTrue

        $script:State = Set-FirstRegistrationVerificationStateStatus `
            -State $script:State `
    ...
    It 'rejects a tampered invocation binding without returning file contents' {
        ...
        $message = ''
        try { Read-FirstRegistrationVerificationState -Path $path -Bindings $script:Bindings | Out-Null }
    ...
    It 'rejects unknown properties malformed JSON and oversized state' {
        ...
        { Read-FirstRegistrationVerificationState -Path $path -Bindings $script:Bindings } |
            Should -Throw '*unexpected property set*'
