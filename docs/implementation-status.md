...
the Job deployment, binds that intent as the sole literal environment entry in the
independently validated template, revalidates the exact Job and zero executions
immediately before the sole no-override start, temporarily assigns the Job identity
as the singular SQL Entra administrator, and restores the exact original
administrator after the exact execution settles. Resume adopts exact dormant or
execution state and never starts a second execution after an unknown outcome.
Initialization accepts only zero user
...
for resource group `rg-a365-custom-gw-phase6t`, deployment ownership
...
including four successful immutable QuickRuns, foundation deployment correlation
`56a563eb-1a48-4ae6-ae31-0acd9e4587dc`, and inert deployment correlation
`be55cb4b-1970-4902-8fed-94c594206e10`.

Step 11 deployed and exactly validated dormant manual Job
`job-a365gw21-db-init-dev` through deployment correlation
`7bae8412-e580-437b-ad29-38d3a1c04ed8`. The durable receipt recorded the Job
principal, verified deployment, administrator-swap intent and completion, and one
Job-start intent. Azure Activity Log correlation
`bbca4fbf-1b0f-4fc2-9802-442faa92a1c5` then failed the start request with safe code
`ContainerAppImageRequired`: supplying only an environment override replaced the
entire execution template and therefore omitted its required image. Exact readback
proved zero Job executions and no database schema/evidence/completion. A bounded
recovery restored the singular original SQL Entra administrator exactly, re-proved
SQL public access `Disabled` and zero firewall rules, and terminated only the local
waiter. Step 11 remains `Running` in its immutable state because that waiter was
terminated after restoration; the partial receipt and live state must both be
preserved. No Admin credential, runtime activation, registration, Registry action,
Gateway key, canary, or Service Bus message-data-plane access followed.

Microsoft's current Container Apps Job contract states that any start override
replaces the full execution template. The working-tree correction therefore binds
the already-persisted non-secret intent into the exact digest-pinned Job template,
revalidates that template and zero executions immediately before the SQL authority
window, and starts the Job with only its exact resource group and name. The
correction passed the complete local gate and must be frozen before a new Plan. That
gate is a zero-warning/zero-error Release build and **1,325/1,325** direct Release tests:
unit 495, Admin UI 155, local Setup 75, observability/runtime 149, integration 92,
...
database/final-verification/experience coverage passes **173/173**, and the Job
architecture tests pass **5/5**. `dotnet format --verify-no-changes` and `git diff
--check` pass. Independent read-only review found no remaining correctness or
safety blocker. Never Resume `a365gw21` with this edited source. The next live
generation is `a365gw22` in resource group
...
Generation `a365gw25` then accepted Plan
...
steps 1–10, then step 11 `Gateway database` became terminal `Failed`; all eight
later steps remained `Pending`, state had no outputs, and overall progress remained
10/19 (52%). Foundation deployment
`a365gw-a365gw25-bootstrap-foundation-dev`, inert deployment
`a365gw-a365gw25-bootstrap-inert-dev`, SQL-private deployment
`a365gw-a365gw25-bootstrap-sql-private-dev`, and database-Job deployment
`a365gw-a365gw25-bootstrap-database-job-dev` all reached their durable verified
checkpoints. The database-Job deployment correlation was
`fc364be9-9d2b-4a66-b471-74966c39a766`.

The exact digest-pinned images were API
...
The source-bound seed blueprint object/application ID is
`0e0a2037-78c3-4ad4-bdc0-c26d8801f12e`; it retained the signed-in administrator
as sole owner/sponsor, exactly one reviewed manager application, and no credential.
The Gateway API application object/client/service-principal IDs are
`55bdaea0-6f89-4db5-bf5f-e9ce2583ce4a`,
`e89da6a3-aa3d-49f8-b6ec-1f7d8c791e69`, and
`62b2d783-70b1-4a99-9249-7e1458e70d94`.

Step 11 deployed manual Job `job-a365gw25-db-init-dev`, principal
`ee83216d-f4a5-4ed3-ab23-1c1e02c1665f`, with execution intent
`b2eb0aff-fd26-4b0e-9a1b-d51562e563ed`. Its single no-override start used
...
with no reported `endTime`. Exact safe console evidence reported SQL error 47073
at `Program.cs` line 233, `connection.OpenAsync()`. That call precedes every
initialize/bootstrap branch and every SQL statement, so this execution wrote no
durable initialization marker, EF schema, or runtime principal and produced no
success/evidence/completion receipt.

The failure was independent of the `a365gw24` catalog baseline. The SQL private-
endpoint control-plane readback had not required the private-endpoint NIC and
private-DNS A record, followed by in-container resolver convergence, before granting
the one Job start. The connection therefore reached the public-routing boundary and
was denied because public access remained disabled. `a365gw24` remains the catalog-
classification checkpoint; `a365gw25` is the distinct DNS-readiness-race checkpoint.
Neither result authorizes relaxing the exact zero-object pristine gate.

The SQL-administrator swap and restoration used correlations
`e29b299d-07f1-4a5d-8fb7-fef6108eae2a` and
`e7fdb10d-2d67-4d30-ac64-84a1c0bdd050`. The receipt records restoration at
`2026-08-30T14:47:10.7369950+00:00`, and exact post-failure readback proved the
singular original administrator restored: `admin@diax48836189.onmicrosoft.com`,
object ID/SID `2db7287c-9462-404f-810e-17e57377618d`, tenant
`ff8b1e46-ff0f-4bc2-ab02-caf2b92da496`. SQL was `Ready`, public network access was
`Disabled`, and the server had zero firewall rules, zero sensitivity
classifications, disabled server/database auditing with no Azure Monitor audit
target, and disabled threat policy. The API and worker remained on their inert
digests: API admission, delegated action, continuous access, and database
attestation were false; worker processing, provisioning execution, outbox relay,
and Direct Registry preview were false, with Registry provider `Disabled`. No Admin
UI identity/credential, runtime/Admin deployment, registration, Registry action,
Gateway key, canary, or Service Bus data-plane action followed.

Preserve the immutable accepted snapshot, state, receipt, diagnostic, Job execution,
...
`a365gw25`; its accepted source is frozen. After a separately reviewed DNS-readiness
fix, the next isolated generation is `a365gw26` /
`rg-a365-custom-gw-phase6y`, only in target subscription
`6f6ae863-dcb7-456f-a7f0-d6f9887cfb76`.

The DNS-readiness correction is now complete in local source. The management-plane
gate derives the sole private-endpoint NIC and canonical private IPv4 address,
requires the exact private-DNS A-record set to contain only that address, persists
the five-field NIC/A-record tuple in the schema-v2 recovery receipt, and re-reads
the complete tuple immediately before temporary SQL-administrator elevation. The
Job receives the expected private-endpoint address through its exact immutable
argument list. Before token acquisition or SQL connection open, the bootstrap-only
migrator gate waits within a bounded ten-minute window until the container resolver
returns exactly that one RFC1918 address. It never retries SQL, migration, Job
start, or another external mutation; the Job remains manual, single-execution, and
`replicaRetryLimit: 0`.

The zero-warning/zero-error Release build and all **1,439/1,439** direct Release
...
for `rg-a365-custom-gw-phase6z`, deployment ownership
...
from clean commit `8a7b4d47090866c4505a36c32bfbc8084e1eb6d9`. What-If contained
exactly eight Creates and zero Deletes, only in target subscription
`6f6ae863-dcb7-456f-a7f0-d6f9887cfb76`. Apply completed steps 1–10.
Foundation, inert, SQL-private, and database-Job deployment correlations were
`b80515c8-b1c7-460a-81b1-5a55f49ae6e6`,
`b60e6133-d780-4afa-8856-75f89b5ec562`,
`7321e1e4-73c4-4d9c-80fa-399bb9c73966`, and
`2f23facc-d3d9-4c80-80c2-0ab2a0100344`. API, worker, Admin UI, and database-
migrator image digests were respectively
...
Step 11 deployed manual Job `job-a365gw27-db-init-dev`, principal
`3c9f501b-fa91-480f-a690-8868cef4e4ed`, with intent
`90df820b-807a-4cfe-aaa1-a154dd66eb33`. Its sole start created execution
`job-a365gw27-db-init-dev-zkl60vp` and replica
`job-a365gw27-db-init-dev-zkl60vp-khcnm`. Both DNS gates passed and SQL opened
privately, but the original 81-counter query referenced its CTE 81 times and hit
SQL command timeout `-2` before returning a row. The execution is terminal
`Failed` with no reported `endTime`. It wrote no initialization marker, EF table,
or runtime principal. The exact original SQL Entra administrator was restored;
public access remained `Disabled` and the server retained zero firewall rules.

Five separate retry-disabled diagnostic Jobs then each started exactly once under
the same private-DNS, session-lock, temporary-administrator, and read-only
diagnostic boundary. None reused or restarted the original Job:

| Diagnostic | Exact execution | Terminal result | Safe outcome |
|---|---|---|---|
| `job-a365gw27-db-diag-dev` | `job-a365gw27-db-diag-dev-j7xpziz` | `Failed`, as expected | Fast aggregate isolated one enabled audit specification, one exact database-level `dbo` `CONNECT` Grant from `dbo`, and one positive-ID Microsoft-shipped `sys` view receiving public `SELECT`. |
| `job-a365gw27-db-diag2-dev` | `job-a365gw27-db-diag2-dev-gvnux1l` | `Failed`, as expected | Proved the view was exactly `sys.database_firewall_rules`; the audit specification was enabled, had a non-null GUID, had zero detail rows, and matched neither the server nor database name. |
| `job-a365gw27-db-diag3-dev` | `job-a365gw27-db-diag3-dev-0p4h1sb` | `Failed`, as expected | Recorded only the audit-specification name fingerprint `sha256:e0f4f7f5e21d49507cf14e0bf1bc6f6b43e7085aaf424fc68e81b33e4ff2ec26`. |
| `job-a365gw27-db-diag4-dev` | `job-a365gw27-db-diag4-dev-vkqz2us` | `Failed`, as expected | The audit row and `dbo` `CONNECT` row matched the corrected contract; the remaining firewall-view Grant was rejected because its grantor had not yet been bound exactly. |
| `job-a365gw27-db-diag5-dev` | `job-a365gw27-db-diag5-dev-vmd3wdp` | `Succeeded` | Proved the firewall-view Grantor is exactly `sys` and passed the complete pristine count-only contract; diagnostic mode returned before any database mutation. |

The exact observed inert service profile is therefore: zero user tables; the one
enabled, non-zero-GUID, zero-detail audit specification identified only by the
fingerprint above; one database-class, database-address, ordinary `CONNECT` Grant
to `dbo` from `dbo`; one object-level ordinary `SELECT` Grant to `public` on the
Microsoft-shipped `sys.database_firewall_rules` view from `sys`; and zero other
unexpected catalog, schema, principal, role-membership, permission, option, or
owner-mismatch rows. The original SQL administrator was restored after every Job.
No initialization marker, schema, runtime principal, Admin UI identity/credential,
runtime activation, registration, Registry action, Gateway key, canary, or Service
Bus data-plane action followed.

Preserve the accepted snapshot, state, original receipt, five diagnostic receipts,
six exact executions, replicas, and resources under
...
`a365gw27` and never second-start any of its six Jobs. The accepted generation is
frozen before the final-profile source changes. At that checkpoint, the only safe
next deployment was a fresh `a365gw28` Plan/Apply in target subscription
`6f6ae863-dcb7-456f-a7f0-d6f9887cfb76`; protected subscription
`95bedc30-f6ac-481b-a3a6-588d2883c216` remains outside this proof.

Generation `a365gw28` then accepted Plan
...
from clean commit `355a0e9caa30fd32b06148d379cbbe03074a5b26`. What-If
contained exactly eight Creates and zero Deletes in target subscription
`6f6ae863-dcb7-456f-a7f0-d6f9887cfb76`, and Apply completed steps 1–10.

The retry-disabled step-11 Job `job-a365gw28-db-init-dev`, principal
`0e64dcd1-1dc3-40bc-941e-71999e56d799`, with intent
`eaf99652-2353-4f43-8cce-6d75e01f70e4`, started exactly once as execution
`job-a365gw28-db-init-dev-x5wjbrl`. Its first pristine read returned the
audit-specification mismatch category as one, so the Job failed closed before any
initialization marker, schema/table, or runtime-principal mutation. The exact
original administrator `admin@diax48836189.onmicrosoft.com`, object/SID
`2db7287c-9462-404f-810e-17e57377618d`, tenant
`ff8b1e46-ff0f-4bc2-ab02-caf2b92da496`, was restored at
`2026-08-30T18:01:38.5087180Z`. Every later Apply step remained pending and the
API/worker remained inert.

A separate retry-disabled, read-only diagnostic Job then started exactly once as
`job-a365gw28-db-diag-dev-u7mt9mi` and reached `Succeeded` at 18:06 UTC. It returned
before database mutation, restored the original administrator, and observed the
same enabled, non-null-GUID, zero-detail audit specification and name fingerprint
`sha256:e0f4f7f5e21d49507cf14e0bf1bc6f6b43e7085aaf424fc68e81b33e4ff2ec26`
as `a365gw27`, together with the exact `dbo`-from-`dbo` database `CONNECT` and
`sys`-granted public `SELECT` on `sys.database_firewall_rules`. This proves the
original failure was transient audit-specification readiness in this run; two
deployments reproducing the fingerprint still do not make it a documented Azure
SQL portability guarantee. Bootstrap currently verifies only its
`SQLSecurityAuditEvents` diagnostic setting and does not implement an independent
server/database audit-policy readback.

Preserve both `a365gw28` Jobs; never Resume `a365gw28` or second-start either Job.
Corrected local source now performs dedicated audit-specification readiness
convergence under the existing SQL initialization lock. Only a pristine surface
whose sole mismatch is the one audit-specification category may enter the wait;
any mixed or other mismatch still fails closed. The wait reads only the compact
exact tuple for total rows, expected-name hash match, enabled/disabled state,
null/zero/non-zero GUID, and detail count. Its monotonic ten-minute wall-clock
deadline covers both reads and delays, and caller cancellation propagates through
both. After readiness, bootstrap rereads and asserts the complete pristine surface
immediately before writing the marker and creating schema.

Independent review found no blocker. Frozen-source local proof passes focused C#
**148/148**, focused `SqlNetwork.Tests.ps1` **14/14**, database-migrator Release
with zero warnings/errors, full solution Release with zero warnings/errors, and
full .NET **1,482/1,482** with zero failed or skipped
...
zero failed, **1** skipped, and zero inconclusive/not-run. This is local source
proof, not live convergence evidence. At that checkpoint, the next deployment was
a fresh isolated `a365gw29` Plan/Apply only in target subscription; `a365gw28`
remained frozen.

Generation `a365gw29` then accepted Plan
...
from commit `454432a590b91ef1d04f057c9d52624b49e87d19`. What-If contained
exactly eight Creates and zero Deletes in target subscription
`6f6ae863-dcb7-456f-a7f0-d6f9887cfb76`; Apply completed steps 1–10.

Step 11 deployed retry-disabled Job `job-a365gw29-db-init-dev`, principal
`bcf6412d-6f15-49e9-99c3-40657873119f`, with intent
`5eba41af-155d-4d43-93b8-f2cbe7f39302`. Its sole execution
`job-a365gw29-db-init-dev-filge13` reached the bounded audit convergence, passed
the complete pristine assertion, wrote the initialization marker, and created the
schema. Post-create exact-schema verification then threw the EF Core 10 runtime-
model error requiring `DbContext.GetService<IDesignTimeModel>().Model` in
`Program.cs` `GetExpectedSchemaContract` / `IsTableExcludedFromMigrations`. The
execution is terminal `Failed` and must never be replayed. Failure occurred before
runtime-principal creation or completion evidence; every later Apply step remained
pending and the API/worker stayed inert.
...
activation, registration, Registry action, Gateway key, canary, or Service Bus
data-plane action followed. Preserve the accepted snapshot, state, receipt,
execution, marker/schema, and resources; never Resume `a365gw29` or second-start
its Job.

Corrected local source makes the exact two-line production change in
`GetExpectedSchemaContract`: relational enumeration is now rooted at
`context.GetService<IDesignTimeModel>().Model`. The closed-connection regression
directly enumerates the design-time model's `Tables` and
`IsExcludedFromMigrations` values and structurally forbids `context.Model` in that
path. Independent review found no blocker and confirmed that adjacent collation,
check-constraint, identity, and index metadata remain correct.

Focused regression **1/1** and the full phase class **31/31** pass. The full Release
solution builds with zero warnings/errors. Full .NET passes **1,483/1,483** with
...
6. For a new clean subscription, start at
   ...
   admit one unknown catalog object. Preserve `a365gw25` with steps 1–10 complete,
   step 11 terminal `Failed`, exactly one failed Job execution/replica, SQL error
   ...
   catalog class. Its count-only correction remained fail-closed. `a365gw25` then
   failed at SQL `OpenAsync` with error 47073 before any statement because private-
   ...
   Preserve `a365gw27` with steps 1–10 complete, the original step-11 Job terminal
   `Failed` after its single query timed out, and five separate diagnostic Jobs each
   started exactly once. Diagnostic 5 alone reached `Succeeded` and proved the
   exact read-only inert Azure SQL profile recorded above. Every Job restored the
   exact original administrator, and no marker, table, runtime principal, or later
   bootstrap mutation occurred. Never Resume `a365gw27` or second-start any of its
   six Jobs.
   Preserve `a365gw28` with steps 1–10 complete, its sole original step-11
   execution terminal `Failed` on the transient audit-specification mismatch, and
   the separate read-only diagnostic execution `job-a365gw28-db-diag-dev-u7mt9mi`
   terminal `Succeeded` after reproducing the exact `a365gw27` service profile.
   Both returned before database mutation and restored the exact original
   administrator; every later step remained pending and the API/worker remained
   inert. Never Resume `a365gw28` or second-start either Job.
   Preserve `a365gw29` with steps 1–10 complete and its sole retry-disabled step-11
   execution terminal `Failed` after audit convergence, marker write, and schema
   creation. Its EF Core 10 post-create exact-schema check failed before runtime-
   ...
   reviewed Container Apps Job readback normalization. The bounded read-only
   pristine-profile convergence fix is implemented and passes the frozen-source
   local gates recorded above, including canonical Bootstrap Pester **465/466**
   with zero failed, one skipped, and zero inconclusive/not-run in **192.48s**.
